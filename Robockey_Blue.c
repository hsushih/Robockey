...
#define CHANNEL 1
//Robot 1
#define RXADDRESS 0x51
#define TXADDRESS 0x50
    ...
    while(1)
    {
		...
		switch(buffer[0]){
			...
			case 0xA1:
				...
				else if(goal_behind ==2){
					find_puck_behind_right();
				}
				else if(goal_behind ==3){
					test2_behind_right();
				}
				else if(goal_behind ==4){
					test2_behind_left();
				}
				else if(goal_behind ==0) {
					//clear(PORTB,0);
					find_puck();
				}
				if (goal_behind !=0){
		...
		m_usb_tx_int(robot_angle);
		m_usb_tx_string("   ");
		m_usb_tx_string("x= ");
		m_usb_tx_int(robotPosition[0]);
		m_usb_tx_string("   ");
		m_usb_tx_string("y = ");
		m_usb_tx_int(robotPosition[1]);
		m_usb_tx_string(" \n  ");
		m_wait(30000);*/
...
void test2_behind_left(){
	...
	if(robot_angle < -90 && robot_angle > -179 && goal_behind == 4 && rotate_half_circle == 0 && turnright == 1){
		determine = 2;
		turnright = 0;
...
void test2_behind_right(){
	...
	if(robot_angle < -1 && robot_angle > -90 && goal_behind == 3 && rotate_half_circle == 0 && turnright == 1){
		determine = 2;
		turnright = 0;
		rotate_half_circle = 1;
	}
...
void find_puck_behind_right(){
	...
	if(robot_angle < 90 && robot_angle > 0 && goal_behind == 2 && rotate_half_circle == 0 && turnright == 1){
		determine = 2;
		turnright = 0;
		rotate_half_circle = 1;
	}
...
void find_puck_behind(){
	...
	if(robot_angle < 179 && robot_angle > 90 && goal_behind == 1 && rotate_half_circle == 0 && turnright == 1){
		determine = 2;
		turnright = 0;
		rotate_half_circle = 1;
	}
...
void checkRobotPosition(){
	//Defend Goal B
	if(test == 1 && robotPosition[0] > 75 && (robot_angle > 0 && robot_angle < 180) && defend == 1){
		defend_num = 2;
	}
	/*else if(test==2 && robotPosition[0] < -75 && (robot_angle < 0 && robot_angle > -180 )){
		defend_num = 3;
	}*/
	else if(test==1 && robotPosition[0] < 75 && defend == 1){
		defend_num = 1;
	}
	else if (defend == 0){
		OCR1B = 0;
